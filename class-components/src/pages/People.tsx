...
import { IPeopleResult, IPeople } from '../types/types';
...
export default function People() {
  ...
  function getNextPageUrl() {
    const nextPage = SWAPIData?.next
      ? SWAPIData?.next.slice(SWAPIData?.next.lastIndexOf('/'))
      : '';
    return `/people${nextPage}`;
  }
  function getPrevPageUrl() {
    const prevPage = SWAPIData?.previous
      ? SWAPIData?.previous.slice(SWAPIData?.previous.lastIndexOf('/'))
      : '';
    return `/people${prevPage}`;
  }

  function detailsRender(): JSX.Element {
    if (details === null) {
    ...
    <MainLayout>
      <>
        ...
        <section className="result-wrapper">
          <PeopleList
            people={SWAPIData?.results || []}
            setDetails={setDetails}
          />
          {detailsRender()}
        </section>
        <div>
          <Link to={getPrevPageUrl()} onClick={() => onPrev()}>
            Prev
          </Link>
          <Link to={getNextPageUrl()} onClick={() => onNext()}>
            Next
          </Link>
        </div>
      </>
